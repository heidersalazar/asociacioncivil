...
$("#incluir").text("Agregar Vigilante");
$("#encabezado").html('<tr>'+
			'<th>Acciones</th>'+
		    '<th>Cedula</th>'+
			'<th>Apellidos</th>'+
...
//VALIDACIONES PARA DIRECCION	
    $("#direccion").prop("maxLength","200");
	$("#direccion").on("keypress", function(e){
		er = /^[0-9A-Za-z\s]*$/; 
		validarkeypress(er,e);
	});
	$("#direccion").on("focus",function(){
		$("#sdireccion").text("Solo letras y numeros entre 3 y 200 ");
	});
	$("#direccion").on("keyup", function(){
		er = /^[0-9A-Za-z\s\b]{3,200}$/;
		r = validarkeyup(er,$(this),$("#sdireccion"),"Solo letras y numeros entre 3 y 200 caracteres ");
	});
//FIN DE VALIDACIONES DE DIRECCION
...
function valida_datos(){ 
	...
	er = /^[0-9A-Za-z\s\b]{3,200}$/;
	r = validarkeyup(er,$("#direccion"),$("#sdireccion"),"Solo letras y numeros entre 3 y 200 ");
	if(r==0){ //r==0, indica que hubo error en la validacion de la etiqueta 
	   error = "ERROR <br/>La Dirección debe contener <br/>Solo letras y numeros entre 3 y 200 ";   
	   return error; //al colocar return aca, sale de la funcion y no ejecuta el resto	
	}
	...
	if(r==0){ //r==0, indica que hubo error en la validacion de la etiqueta 
	   error = "ERROR <br/>El telefono debe contener <br/>Solo numeros entre 11 y 15 digitos ";
	   return error; //al colocar return aca, sale de la funcion y no ejecuta el resto	
	}
...
function pone(a,b){
	...
	$("#cedula").val($(p).find("td:eq(1)").text());
	$("#nombres").val($(p).find("td:eq(2)").text());
	$("#apellidos").val($(p).find("td:eq(3)").text());
	$("#telefono").val($(p).find("td:eq(4)").text());
	$("#correo").val($(p).find("td:eq(5)").text());
	$("#telefono").val($(p).find("td:eq(6)").text());
	$("#direccion").val($(p).find("td:eq(7)").text());
	if(b==1){
		ope = "modificar";
		$("#ejecutar").text('Modificar');
		$("#titulo").text('Modificar Vigilante');
		$("#cedula").prop("disabled",false);
		$("#apellidos").prop("disabled",false);
		$("#nombres").prop("disabled",false);
		$("#telefono").prop("disabled",false);
		...
		$("#titulo").text('Eliminar Vigilante');
